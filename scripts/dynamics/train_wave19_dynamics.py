#!/usr/bin/env python3
"""Train Wave-19 LIBERO F1/F2 and run the frozen offline replication gate.
...
PYTHONPATH=src CUBLAS_WORKSPACE_CONFIG=:4096:8 \
  ...
  scripts/dynamics/train_wave19_dynamics.py \
  --config configs/dynamics_7.yaml --device cuda:0

Outputs
-------
Writes semantic/F1/F2 checkpoints, frozen train/development latents, offline
metrics/gate/report, and ``wave19_frozen_model_manifest.json`` below the
Wave-19 results/data roots. If the offline gate fails, closed-loop execution
...
def main() -> None:
    ...
    write_json(
        out / "wave19_dynamics_disk_record.json",
    ...
    gate = json.loads((out / "wave19_representation_gate.json").read_text(encoding="utf-8"))
    if gate.get("representation_ready_LIBERO") is not True:
        raise RuntimeError("Representation R-gate did not pass")
    selected_manifest = json.loads((out / "wave19_selected_representation_manifest.json").read_text(encoding="utf-8"))
    split = json.loads((out / "wave19_dataset_split_manifest.json").read_text(encoding="utf-8"))
    device = torch.device(args.device)
    if device.type != "cuda" or not torch.cuda.is_available():
        raise RuntimeError("Wave-19 dynamics training requires CUDA")
    ...
    text_features = np.asarray(
        np.load(data / "derived/representation/text_features.npz")["features"], dtype=np.float32
    )
    ...
    write_json(out / "wave19_offline_replication_gate.json", offline)
    ...
    report = f"""# Wave-19 LIBERO dynamics results
...
All learned models were initialized independently from CALVIN and trained only on the Wave-19 train split.
"""
    (out / "wave19_dynamics_results.md").write_text(report, encoding="utf-8")
    model_manifest = {
        "frozen_at": now(),
    ...
    write_json(out / "wave19_frozen_model_manifest.json", model_manifest)
    with (out / "exact_commands.sh").open("a", encoding="utf-8") as handle:
        handle.write(
            f"\n# {now()} phase=dynamics-training-and-offline-gate\n"
            "PYTHONPATH=src CUBLAS_WORKSPACE_CONFIG=:4096:8 "
            "/home/jinjaguo/anaconda3/envs/libero/bin/python "
            "scripts/dynamics/train_wave19_dynamics.py --config configs/dynamics_7.yaml --device cuda:0\n"
        )
    if not accepted:
        (out / "wave19_offline_replication_rejection.md").write_text(
            "# Wave-19 offline replication rejected\n\nAt least one frozen O1–O8 condition failed. "
            "Per protocol, exact-state closed-loop B0–B5 evaluation did not run.\n",
            encoding="utf-8",
        )
        raise RuntimeError("Wave-19 offline replication gate failed")

#!/usr/bin/env python3
"""Collect and certify fresh Wave-20 π0.5 confirmation trajectories.
...
25/50/75-percent branches by twin replay, and freeze exactly five new certified
confirmation-development episodes per task without touching the Wave-19 test.

Parameters
----------
``--config`` selects the frozen Wave-20 YAML. ``--host`` and ``--port`` select
the already-running Wave-20 π0.5 websocket server. Collection scale, seeds,
...
PYTHONPATH=src:/home/jinjaguo/LIBERO:/home/jinjaguo/openpi/packages/openpi-client/src \
  ...
  scripts/dynamics/collect_wave20_libero.py \
  --config configs/dynamics_8.yaml --host localhost --port 8000

Outputs
-------
Immutable raw episodes are finalized below
``data/wave20_libero_confirmation/raw_collection``. Certified episodes/branches
are written below ``data/wave20_libero_confirmation/certified``. Collection,
snapshot-certification, and confirmation manifests/reports are written below
``results/dynamics/twentieth_wave/2026-08-14_dynamics_8``.
"""
...
def begin_episode(
    *,
    ...
    runtime = EpisodeRuntime(
        task_id=task_id,
        attempt=attempt,
        episode_id=f"wave20_task{task_id:02d}_attempt{attempt:03d}",
...
def certify_branch(
    ...
    try:
        ...
        twin_b.close()
    frozen = json.loads(
        (ROOT / config["experiment"]["output_root"] / "wave20_snapshot_certification_preregistration.json").read_text(
            encoding="utf-8"
        )
    )
...
def certify_success(
    ...
    for candidate in eligible:
        ...
        if not passed:
            ...
            write_json(out / "wave20_snapshot_certification_failure.json", result)
            write_text(
                out / "wave20_reconstruction_gate_failure.md",
                "# Wave-20 reconstruction gate failure\n\nA prospective source branch failed the frozen exact-state "
...
def existing_certified(data: Path, task_id: int) -> list[str]:
    ...
    return sorted(
        path.name
        for path in root.glob("wave20_task*_attempt*")
        if (path / "episode_metadata.json").is_file()
    )
    ...
    *,
    ...
    while active:
        ...
        if needing:
            ...
            response = client.infer({"wave20_observation_batch": requests})
            results = response["wave20_batch_results"]
...
def freeze_confirmation(config: Mapping[str, Any], collection_rows: Sequence[Mapping[str, Any]]) -> None:
    out = ROOT / config["experiment"]["output_root"]
    data = ROOT / config["experiment"]["data_root"]
    ...
    if any(len(values) != target for values in certified_by_task.values()):
        raise RuntimeError(f"Fresh confirmation must freeze exactly {target} certified episodes per task")
    episodes = []
    for task_id, values in certified_by_task.items():
        for episode_id in values:
            ...
            branch_metadata_hashes = {
                branch.parent.name: sha256_file(branch)
                for branch in sorted((certified_path / "branches").glob("*/branch_metadata.json"))
            }
            branch_snapshot_hashes = {
                snapshot.parent.name: sha256_file(snapshot)
                for snapshot in sorted((certified_path / "branches").glob("*/exact_snapshot.pkl"))
            }
            actions = np.load(certified_path / "actions.npy", mmap_mode="r")
            episodes.append(
                {
                    "episode_id": episode_id,
                    "task_id": task_id,
                    "task_label": metadata["instruction"],
                    ...
                    "branch_metadata_sha256": branch_metadata_hashes,
                    "branch_snapshot_sha256": branch_snapshot_hashes,
                    "environment_seed": metadata["environment_seed"],
                    "task_seed": metadata["task_seed"],
                    "policy_seed": int(config["experiment"]["seed"]),
                    "terminal_official_success": metadata["terminal_official_success"],
                    "length": int(actions.shape[0]),
                    "action_count": int(actions.shape[0]),
                    "eligible_branch_count": metadata["eligible_branch_count"],
                }
            )
    if len({row["episode_id"] for row in episodes}) != len(episodes):
        raise RuntimeError("Episode ID appears more than once in fresh confirmation set")
    wave19 = json.loads(
        (ROOT / config["sources"]["wave19_output_root"] / "wave19_dataset_split_manifest.json").read_text(
            encoding="utf-8"
        )
    )
    wave19_ids = {row["episode_id"] for row in wave19["episodes"]}
    if wave19_ids.intersection(row["episode_id"] for row in episodes):
        raise RuntimeError("Fresh confirmation episode ID overlaps Wave-19")
    write_json(
        out / "wave20_fresh_confirmation_manifest.json",
        {
            "frozen_at": now(),
            "suite": "libero_10",
            "role": "Wave20_fresh_confirmation_development_set",
            "certified_episodes_per_task": target,
            "total_certified_episodes": len(episodes),
            "disjoint_from_all_wave19_episode_ids": True,
            "not_added_to_training": True,
            "not_added_to_wave19_final_test": True,
            "wave19_final_test_read": False,
            "episodes": episodes,
...
def main() -> None:
    ...
    out = ROOT / config["experiment"]["output_root"]
    data = ROOT / config["experiment"]["data_root"]
    if not (out / "wave20_snapshot_certification_preregistration.json").is_file():
        raise RuntimeError("Development snapshot gate must be frozen before source collection")
    if (out / "wave20_fresh_confirmation_manifest.json").exists():
        raise RuntimeError("Wave-20 confirmation set is already frozen; refusing additional collection")
    ...
    execution_manifest = {
        ...
        "jit_mode": config["collection"]["jit_mode"],
        "numba_disable_jit_environment_variable": os.environ.get("NUMBA_DISABLE_JIT"),
        "batch_padding_result_discarded_when_only_one_episode_active": True,
    }
    execution_path = out / "wave20_collection_execution_manifest.json"
    ...
    with (out / "exact_commands.sh").open("a", encoding="utf-8") as handle:
        handle.write(
            ...
            "PYTHONPATH=src:/home/jinjaguo/LIBERO:/home/jinjaguo/openpi/packages/openpi-client/src "
            "MUJOCO_GL=egl /home/jinjaguo/anaconda3/envs/libero/bin/python "
            "scripts/dynamics/collect_wave20_libero.py --config configs/dynamics_8.yaml "
            f"--host {args.host} --port {args.port}\n"
        )
    client = WebsocketClientPolicy(args.host, args.port)
    metadata = client.get_server_metadata()
    if metadata.get("wave20_batch_inference") is not True:
        raise RuntimeError("Connected server does not expose frozen Wave-20 batch inference")
    if int(metadata.get("wave20_policy_seed", -1)) != int(config["experiment"]["seed"]):
        raise RuntimeError("Connected source-policy seed differs from frozen Wave-20 seed")
    ...
    for task_id in range(suite.get_num_tasks()):
        ...
        for raw_path in sorted(raw_success_root.glob("wave20_task*_attempt*")):
        ...
        while len(certified) < target and len(attempted) < maximum_attempts:
            ...
            pair_attempts = available[: min(2, target - len(certified))]
            runtimes = run_pair(
                task_id=task_id,
        ...
        if len(certified) < minimum:
            write_text(
                out / "wave20_source_collection_failure.md",
                f"# Wave-20 source collection failure\n\nTask {task_id} produced only {len(certified)} certified "
    ...
    write_json(
        out / "wave20_snapshot_certification_results.json",
        {
            "created_at": now(),
    ...
    write_json(
        out / "wave20_data_collection_summary.json",
        {
            "created_at": now(),
    ...
    report_lines = [
        "# Wave-20 π0.5 source-data collection",
        "",
    ...
    write_text(out / "wave20_data_collection_report.md", "\n".join(report_lines))
    freeze_confirmation(config, all_progress)
